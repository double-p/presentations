...
format:
  revealjs:
    ...
    footer: "BSDCan 2023 - Oddawah - Jitsi/OpenBSD"
...
- `/etc/rc.conf.local`:
```{.bash}
rcctl set jicofo flags "--host=jitsi.fips.de"
```
  ...
  xmpp-client {
   configs {
    ourprosody {
     ...
     domain = "auth.jitsi.fips.de" // 'realm', no DNS
     username = "jvb"
...
### fips.de
[`https://jitsi.eurobsdcon.org/BSDCan`](https://jitsi.eurobsdcon.org/BSDCan)
