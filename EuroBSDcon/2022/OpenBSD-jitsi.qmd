...
## DNS, `/etc/hosts`
:::{.callout-note}
DNS: only one A-RR for jts.fips.de, but `hosts` for jicofo
:::
...
prosodyctl register jvb auth.jts.fips.de CHANGE_JVB
```
...
:::{.callout-important}
Do NOT change `cacerts` - upgrade from jdk-11 to newer would "forget" the keystore
:::
